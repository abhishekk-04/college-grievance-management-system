              ...
              <div style={{ marginTop: '20px', borderTop: '1px solid var(--border)', paddingTop: '16px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                  {grievance.attachments.map((fileUrl, index) => {
                    const filename = fileUrl.split('-').slice(2).join('-') || 'Attachment';
                    const isImg = /\.(jpg|jpeg|png)$/i.test(fileUrl);
                    return (
                      <a 
                        key={index}
                        href={`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}${fileUrl}`} 
                        target="_blank" 
                        rel="noreferrer"
                  ...
                  return (
                    <div key={step} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 2, position: 'relative' }}>
                      <div style={{
                        ...
                        height: '32px',
                        borderRadius: '50%',
                        background: isCurrent ? 'var(--primary)' : isActive ? 'var(--primary-glow)' : 'var(--bg-secondary)',

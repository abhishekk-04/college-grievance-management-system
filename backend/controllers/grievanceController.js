  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      for (const file of req.files) {
        ...
        const filename = `attachments-${uniqueSuffix}${path.extname(file.originalname)}`;
        ...
        if (uploadError) {
          ...
          return res.status(500).json({ message: `Error uploading attachment: ${uploadError.message}` });
        }
